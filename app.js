...
async function loadProducts() {
    try {
        ...
        if (company_id && data.companies) {
            let comp = data.companies.find(c => c.id == company_id);
            if (comp) {
                products = comp.products || [];
                window.allCategories = comp.categories || [];
            } else {
...
function handleCheckout() {
    ...
    fetch('/api/checkout', {
        ...
        body: JSON.stringify({ user_id: parseInt(user_id), items: orderItems, company_id: company_id ? parseInt(company_id) : null })
    })
    .then(res => res.json())
    .then(data => {
        if(data.status === 'ok') {
